...
func (service *UploadTokenService) Post() serializer.Response {
	client, err := oss.New("oss-cn-beijing.aliyuncs.com", "LTAI4GE2CS8jxJwbjJAB1Kvq", "qQgVjCnnak3gteeVbZeYZGNBFVjufT")
	if err != nil {
	...
	// 查看图片
	signedGetURL, err := bucket.SignURL(key, oss.HTTPGet, 600, option...)
	if err != nil {
		return serializer.Response{
			Code:  50002,
			Msg:   "oss配置失败",
